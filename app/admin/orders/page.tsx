...
export default async function AdminOrdersPage() {
  ...
  // Email gate — only the admin can see this page
  const adminEmail = process.env.ADMIN_EMAIL;
  if (!adminEmail || user.email !== adminEmail) {
    redirect("/dashboard");
  }

  // Fetch all paid orders with their items, newest first
  ...
  return (
    <main className="min-h-screen bg-creme-paper text-encre">
      <nav className="mx-auto flex max-w-5xl items-center justify-between px-6 py-6">
        ...
        <span className="text-sm text-pierre">Admin · Commandes</span>
      </nav>
      ...
      <div className="mx-auto max-w-5xl px-6 py-8">
        <div className="flex items-end justify-between">
          ...
          <p className="text-sm text-pierre">
            {rows.length} commande{rows.length > 1 ? "s" : ""} payée
            {rows.length > 1 ? "s" : ""}
          </p>
        </div>
                      ...
                      <span
                        className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${
                          order.status === "fulfilled"
                            ...
                            : "bg-corail/15 text-corail-deep"
                        }`}
                      >
                        {order.status === "fulfilled" ? "Expédiée" : "À traiter"}
                      </span>
